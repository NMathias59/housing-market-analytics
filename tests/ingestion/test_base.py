...
class TestLoadDf:
    def test_returns_row_count(self):
        ...
        assert load_df(client, df, "db_wh_housing.raw_eptb") == 3

    def test_calls_insert_df(self):
        client = MagicMock()
        df = pd.DataFrame({"id": [1]})
        load_df(client, df, "db_wh_housing.raw_eptb")
    ...
    def test_passes_correct_table_name(self):
        ...
        assert table_arg == "db_wh_housing.raw_eptb"
    ...
    def test_skips_insert_on_empty_df(self):
        client = MagicMock()
        result = load_df(client, pd.DataFrame(), "db_wh_housing.raw_eptb")
        assert result == 0
